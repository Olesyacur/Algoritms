...
def sleight_of_hand(matrix: str, max_clicks: int) -> int:
    bonus = 0
    for key_digit in range(10):
        key_char = str(key_digit)
        count =matrix.count(key_char)
        if 0 < count <= max_clicks:
            bonus += 1
    return bonus


def main() -> None:
    max_clicks = int(input())*2
    matrix = ''.join(input() for _ in range(4))

    bonus = sleight_of_hand(matrix, max_clicks)
